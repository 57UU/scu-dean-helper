...
export  class ImageClassifier {
    ...
    private static ortSession: ort.InferenceSession;
    ...
    public static async classification(imgElement: HTMLImageElement): Promise<string> {
        ...
        var image = await Image.load(data);
        image = image.resize({ width: Math.round(image.width * (64 / image.height)), height: 64 });
        image = image.grey();
        const imageData = image.data as Float32Array;
        const imageTensor = new Float32Array(64 * 64);
        for (let i = 0; i < imageData.length; i++) {
        imageTensor[i] = imageData[i] / 255;
        }

        // 扩展维度，归一化图像数据
        const imageReshaped = new Float32Array(1 * 1 * 64 * 64);
        for (let i = 0; i < 64 * 64; i++) {
        imageReshaped[i] = (imageTensor[i] - 0.5) / 0.5;
        }

        // 准备ONNX模型输入
        const ortInputs = { input1: new ort.Tensor('float32', imageReshaped, [1, 1, 64, 64]) };
        // 运行模型
        const ortOuts = await this.ortSession.run(ortInputs);

        // 处理模型输出
        const argmaxResult = ortOuts[0].data as Float32Array;
        let result: string[] = [];
        let lastItem = -1;

        for (let i = 0; i < argmaxResult.length; i++) {
        const item = argmaxResult[i];
        if (item === lastItem) {
            continue;
        } else {
            lastItem = item;
        ...
        if (item !== 0) {
            result.push(this.charset[Number(item)]);
        }
        }
